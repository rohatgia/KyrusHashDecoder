var TextEncoder = require('../dependencies/TextEncoder')
var TextEncoding = require('../dependencies/TextEncoding')

//code borrowed and adapted from cryptii's MD5.js file: https://github.com/cryptii/cryptii/blob/master/src/Encoder/Hash/md5.js
...
function decoding(){
  var str1 = ""
  //var str1 = "Who designed both RC5 and MD5?\n"
  var strprevious = ""
  var strOut = ""
  console.log("entered decoding")
  var fs = require("fs");
  var stream = fs.createWriteStream("../puzzles/puzzle2.txt");
  fs.readFile('../puzzles/puzzle.txt','utf-8',function(err, data){
	console.log("entering read file")
	if (err) throw err;
  var lines = data.split('\n')

  //This method is not time complexity efficient after the first question
    ...
    for(var linei = 32;linei<lines.length;linei++){
      line = lines[linei]
	  for(var i = 0;i<17;i++){
	    var test;
		if(i < 10){
...
function getBytes(string) {
	var arr = [];
	arr = TextEncoder.bytesFromCodePoints(getCodePoints(string), 'utf8')
	return arr;
}
  
function getCodePoints(string){
	return TextEncoder.codePointsFromString(string)
}

function makeString(arr){
	var hex = "";
	for(var x = 0;x<arr.length;x++){
		var tmp = arr[x].toString(16)
		for(var y = tmp.length;y < 2;y++){
			hex += "0"
		}
		hex += tmp
	}
